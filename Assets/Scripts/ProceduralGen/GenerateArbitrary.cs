...
public class GenerateArbitrary : MonoBehaviour
{
        ...
        while (i < numberOfObjects)
        {
            while (spawned == false && overflowLimit > 0)
            {
                ...
                if (worldScript.textureMap.GetPixel((x), (y)).b <= c.b) //compares to threshold
                {
                    ...
                    {
                        if (Vector3.Distance(pos, enemyBase.transform.position) < distToAvoid)
                        {
                            isDistant = false;
                            break;
                        }
                    ...
                    {
                        if (Vector3.Distance(pos, wall.transform.position) < distToAvoid)
                        {
                            isDistant = false;
                            break;
                        }
